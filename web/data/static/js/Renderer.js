...
function grMesh(vertexArray, indexArray, primitive) {
    ...
    this.bufferBlendColor = true;
    this.bufferUV = true;
    this.bufferNormals = true;
    this.bufferJointIds = true;
    this.bufferJointIds2 = true;
    this.jointMapping = true;
    this.materialIndex = true;
}
...
function grModel() {
    ...
    this.meshes = [];
    this.materials = [];
    this.matrices = true; // non multiplied by bind pose matrix
    this.matricesInverted = true; // multiplied by bind pose matrix
...
function grTextMesh(text = undefined, x = 0, y = 0, z = 0, is3d = true, charSize = 9.0) {
    this.refs = 0;
    this.position = [x, y, z];
    this.offset = [0.0, 0.0];
    this.textLength = 0;
...
function grController(viewDomObject) {
    ...
    if (typeof grHelper_Grid === 'function') {
        this.helpers.push(grHelper_Grid(2000, 100, 0, 5));
    }
    if (typeof grHelper_LightDirection === 'function') {
        this.helpers.push(grHelper_LightDirection(450));
    }
    ...
    this.renderSettings = {
        backgroundColor: [0.035, 0.050, 0.080, 1.0],
        ambient: 0.42,
        diffuse: 0.85,
        specular: 0.28,
        exposure: 1.08,
        gamma: 2.2,
        lightDirection: [-0.45, -0.75, 0.35],
    ...
    this._shaderUniformCache = {};
    this._nativeUseProgram = undefined;
    this._wrapUseProgram();
...
grController.prototype.onResize = function() {
    var dpr = (this.renderSettings && this.renderSettings.highDpi) ? (window.devicePixelRatio || 1) : 1;
    var w = Math.max(1, Math.floor(gl.canvas.clientWidth * dpr));
    var h = Math.max(1, Math.floor(gl.canvas.clientHeight * dpr));
    if (gl.canvas.width !== w || gl.canvas.height !== h) {
        gl.canvas.width = w;
        gl.canvas.height = h;
    }
    this.rectX = gl.canvas.clientWidth || w;
    this.rectY = gl.canvas.clientHeight || h;
...
/* ========================================================================== */
/* Pro renderer state, shader uniforms and capture utilities                    */
/* ========================================================================== */
grController.prototype._wrapUseProgram = function() {
    if (!gl || this._nativeUseProgram) return;
    var controller = this;
    this._nativeUseProgram = gl.useProgram.bind(gl);
    gl.useProgram = function(program) {
        controller._nativeUseProgram(program);
        controller.applyCommonShaderUniforms(program);
    };
};

grController.prototype._uniform = function(program, names) {
    if (!program) return null;
    var key = program.__gowUniformKey || (program.__gowUniformKey = 'p' + Math.random().toString(36).slice(2));
    if (!this._shaderUniformCache[key]) this._shaderUniformCache[key] = {};
    var cache = this._shaderUniformCache[key];
    for (var i = 0; i < names.length; i++) {
        var name = names[i];
        if (!cache.hasOwnProperty(name)) cache[name] = gl.getUniformLocation(program, name);
        if (cache[name]) return cache[name];
    }
    return null;
};

grController.prototype.applyCommonShaderUniforms = function(program) {
    if (!program || !this.renderSettings) return;
    var rs = this.renderSettings;
    try {
        var u;
        u = this._uniform(program, ['uAmbient', 'uAmbientLight', 'ambientLight', 'u_ambient']);
        if (u) gl.uniform1f(u, rs.ambient);
        u = this._uniform(program, ['uDiffuse', 'uDiffuseStrength', 'diffuseStrength', 'u_diffuse']);
        if (u) gl.uniform1f(u, rs.diffuse);
        u = this._uniform(program, ['uSpecular', 'uSpecularStrength', 'specularStrength', 'u_specular']);
        if (u) gl.uniform1f(u, rs.specular);
        u = this._uniform(program, ['uExposure', 'exposure', 'u_exposure']);
        if (u) gl.uniform1f(u, rs.exposure);
        u = this._uniform(program, ['uGamma', 'gamma', 'u_gamma']);
        if (u) gl.uniform1f(u, rs.gamma);
        u = this._uniform(program, ['uLightDirection', 'lightDirection', 'u_lightDir', 'uSunDirection']);
        if (u) gl.uniform3fv(u, new Float32Array(rs.lightDirection));
        u = this._uniform(program, ['uLightColor', 'lightColor', 'u_lightColor']);
        if (u) gl.uniform3fv(u, new Float32Array([1.0, 0.96, 0.84]));
    } catch (e) {
        if (!this._uniformWarned) {
            console.warn('GoW Pro: common shader uniform setup failed once', e);
            this._uniformWarned = true;
        }
    }
};

grController.prototype.applyRendererState = function() {
    if (!this.renderSettings) return;
    var c = this.renderSettings.backgroundColor || [0, 0, 0, 1];
    gl.clearColor(c[0], c[1], c[2], c[3]);
    if (this.cull) gl.enable(gl.CULL_FACE); else gl.disable(gl.CULL_FACE);
    if (this.renderSettings.autoRotate && this.camera === this.cameraModels) {
        this.cameraModels.rotation[1] += this.renderSettings.autoRotateSpeed;
        this.requireRedraw = true;
    }
    if (this.helpers) {
        for (var i = 0; i < this.helpers.length; i++) {
            this.helpers[i].visible = this.renderSettings.showGrid !== false;
        }
    }
};

grController.prototype.setRenderSetting = function(key, value) {
    if (!this.renderSettings) this.renderSettings = {};
    this.renderSettings[key] = value;
    if (key === 'textureFiltering') this.applyTextureFilteringToScene();
    if (key === 'highDpi') this.onResize();
    this.requestRedraw();
};

grController.prototype.setBackgroundColor = function(hex) {
    if (!this.renderSettings) return;
    if (typeof hex === 'string' && hex[0] === '#') {
        var n = parseInt(hex.slice(1), 16);
        this.renderSettings.backgroundColor = [((n >> 16) & 255) / 255, ((n >> 8) & 255) / 255, (n & 255) / 255, 1];
    } else if (Array.isArray(hex)) {
        this.renderSettings.backgroundColor = hex;
    }
    this.requestRedraw();
};

grController.prototype.applyTextureFilteringToScene = function() {
    var visited = [];
    function visitTexture(t) {
        if (!t || visited.indexOf(t) >= 0 || !t.applyTexParameters) return;
        visited.push(t);
        t.applyTexParameters();
    }
    visitTexture(this.emptyTexture);
    visitTexture(this.fontTexture);
    for (var mi = 0; mi < this.models.length; mi++) {
        var mdl = this.models[mi];
        for (var ma = 0; ma < mdl.materials.length; ma++) {
            var mat = mdl.materials[ma];
            for (var la = 0; la < mat.layers.length; la++) {
                var layer = mat.layers[la];
                for (var tx = 0; tx < layer.textures.length; tx++) visitTexture(layer.textures[tx]);
            }
        }
    }
};
...
grController.prototype.resetCamera = function() {
    this.cameraModels.target = [0, 0, 0];
    this.cameraModels.distance = 100.0;
    this.cameraModels.rotation = [15.0, 135.0, 0];
    this.cameraInterface.target = [0, 0, 0];
    this.cameraInterface.distance = 100.0;
    this.requestRedraw();
};

grController.prototype.capturePng = function(filename) {
    filename = filename || 'gow_browser_capture.png';
    try {
...
(function() {
    var oldApplyTex = grTexture.prototype.applyTexParameters;
    grTexture.prototype.applyTexParameters = function() {
        if (!this.loaded) return;
        gl.bindTexture(gl.TEXTURE_2D, this.txr);
        var mode = (gr_instance && gr_instance.renderSettings) ? gr_instance.renderSettings.textureFiltering : 'smooth';
        ...
        if (this.isFontTexture || mode === 'pixel') {
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, mode === 'pixel' ? gl.NEAREST : gl.LINEAR);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, mode === 'pixel' ? gl.NEAREST : gl.LINEAR);
        } else {
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, mode === 'sharp' ? gl.LINEAR_MIPMAP_NEAREST : gl.LINEAR_MIPMAP_LINEAR);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
            if (ext && mode === 'smooth') {
                var max = gl.getParameter(ext.MAX_TEXTURE_MAX_ANISOTROPY_EXT) || 1;
